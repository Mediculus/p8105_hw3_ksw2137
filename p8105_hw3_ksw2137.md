    ...
    response = 
      factor(response,
             levels = c("Poor",
                        ...
                        "Very good",
                        "Excellent")),
    data_value = data_value/100,                # converts it into the decimal form of "percent"
...
knitr::kable(count_loc_2002)                           # Prints tables for 2002 States
```
...
``` r
knitr::kable(count_loc_2010)                           # Prints tables for 2010
```
...
The resulting table contains 7 states and 15 states in 2002 and 2010
respectively. Under each state is the number of counties within the
state that responded. There are more than twice states that were part of
the respondent in 2010 compared to 2002. We see that FL’s counties
increased significantly from 7 to 41. We also do not see `CT` in the
2010, likely because it had lower participating counties in 2010.
Interestingly, while working on the table, I noticed that there are 49
states in the 2002 data but 51 states in 2010. After investigating, this
is because Montana (MT) started participating in 2004 and Virginia (VA)
in
2007.
...
spaghetti_plot <- brfss_df %>% 
  ...
  mutate(avg_data = avg_data*100) %>%                           # multiply data by 100 to resemble %age
...
on our color legends, it is likely Wyoming (WY). Evaluating further,
Wyoming seems to be consistently on the very low end of excellent
response. This might indicate that the overall health of the population
in Wyoming is less than excellent or the citizens might expect too much
for an “excellent” health. We might also be able to `facet_wrap` the
plot in order to see a better separation between states:
