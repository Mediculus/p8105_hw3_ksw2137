...
count_loc_2010 <- brfss_count_loc %>%                      # assigns a new df for 2002 counts
  filter(year == "2010",                               # filter by year 2002 and counts > 6
         county_per_state > 6
         ) %>%
  pivot_wider(names_from = state,                      # turn to wide form for kable()
              values_from = county_per_state
