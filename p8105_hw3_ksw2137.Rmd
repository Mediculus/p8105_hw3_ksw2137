...
After splitting each state, we can actually see that the outlier we saw earlier was actually West Virginia (WV). We also noticed that Indiana (IN) also has certain dips in its "excellent" response percentage. On the other hand Connecticut (CT), DC, and Massachusetts (MA) seem to be pretty consistent in their high "excellent" responses. 
...
The graph above definitely shows a typical pattern of a person that wakes up at 06:00, and goes to sleep at ~23:00 - 24:00. This is apparent by his increased activity around 06:00 and decrease closer to 24:00. Interestingly, his activity seems to highest around 11:00 and 21:00 across the days. If we do `facet_grid()` however, we could get a more granular pattern.

```{r faceted_accel_plot}
