...
insta_df %>% 
  ...
  mutate(order_dow = 
           lubridate::wday(order_dow + 1, label = T)
         ) %>% 
...
accel_df %>% 
  ...
  group_by(day_id) %>% 
  mutate(
    minute_mark = as.Date(minute_mark)
  ) %>% 
  ggplot(aes(x = minute_mark, y = activity_count, fill = day)) +
  geom_bar(stat = "identity", alpha = 1) +
  guides(color = guide_legend(override.aes = list(size = 5))) +
  theme(legend.position = "right") +
  scale_x_time(labels = "%h",
               breaks = "1 hour",
               minor_breaks = "15 minutes")
  labs(title = "***",
       subtitle = "***",
       x = "***",
       y = "***",
